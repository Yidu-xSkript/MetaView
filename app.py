    ...
    if request.method == 'POST':
        if login():
            if request.json['orderMode'] == "Open":
                order(request.json['symbol'], request.json['orderAction'], request.json['orderType'], float(request.json['entry_price']), float(request.json['tp']), float(request.json['sl']), request.json['comment'], request.json['maxrisk'])
